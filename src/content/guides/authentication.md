...
### Protocol Flow:
1. The client requests protected resources by sending the token in the `Authorization` header:
   ```http
   Authorization: Bearer my_secure_token_123
   ```
2. If the token is missing, invalid, or expired, the server returns a **`401 Unauthorized`** response.
3. According to specifications, the server must include the **`WWW-Authenticate`** header indicating the scheme and realm:
   ```http
