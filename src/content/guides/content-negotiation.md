...
* [**`Accept`**](/headers/accept): The desired media formats (e.g. `application/json`).
* [**`Accept-Encoding`**](/headers/accept-encoding): Supported compression algorithms (e.g. `gzip, br`).
* **`Accept-Language`**: Preferred localized languages (e.g. `en-US, es`).

The server evaluates these headers, selects the best representation, and returns it. It must include the [**`Content-Type`**](/headers/content-type) header in the response indicating the chosen format.
...
* **For Accept failures**: The server should return [**`406 Not Acceptable`**](/status-codes/406).
* **For incoming payloads**: If a client POSTs an unsupported body type (e.g. sending XML to a JSON-only endpoint), the server must return [**`415 Unsupported Media Type`**](/status-codes/415).
