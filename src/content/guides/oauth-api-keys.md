...
When an application needs to act on behalf of a specific user, reading their data or posting on their account, it needs the user's explicit consent without ever handling that user's password. The authorization code grant, combined with PKCE (Proof Key for Code Exchange, RFC 7636), is the standard flow for this in modern APIs, including public clients like mobile and single-page apps that can't safely hold a client secret.
