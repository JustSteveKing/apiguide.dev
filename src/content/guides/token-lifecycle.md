...
Revoking a refresh token revokes the whole delegation, invalidating the access tokens issued under it. Keeping JWT access-token lifetimes short is itself a practical revocation strategy, since the token expires before a revocation list would matter.
