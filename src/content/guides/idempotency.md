...
An API operation is idempotent if making the same request multiple times yields the same result and server state. While HTTP [`GET`](/methods/get), [`PUT`](/methods/put), and [`DELETE`](/methods/delete) are natively idempotent, [`POST`](/methods/post) (typically used for billing, messaging, or resource creation) is not.
...
Clients send a unique identifier (usually a UUID) in the [`Idempotency-Key`](/headers/idempotency-key) request header.
...
3. **Locking Concurrent Calls**: If a second request arrives with the same key while the first is running, return a [`409 Conflict`](/status-codes/409).
...
When API clients experience network timeouts or receive [`429`](/status-codes/429) or [`503`](/status-codes/503) codes, they will attempt to retry the request. To prevent hammering the server, client libraries should implement **Exponential Backoff with Jitter**.
