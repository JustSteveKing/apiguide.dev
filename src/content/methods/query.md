...
`QUERY` is a proposed HTTP method defined by the IETF HTTPBIS working group draft "The HTTP QUERY Method" (`draft-ietf-httpbis-safe-method-w-body`). It's designed to behave like `GET`, which is to say safe, idempotent and cacheable, but with a request body, so a query can carry a large or complex payload (a structured filter, a GraphQL-style query document, a big list of IDs) without cramming it into a URI or query string.
...
* **Body carries the query, not a mutation**: The request body describes what to retrieve, not a change to make. Semantically this stays a read, same as `GET`, so don't repurpose it for writes.
...
* **Cacheable, with caveats**: The draft allows caching similar to `GET`, but since the "identity" of the request now includes the body (not just the URI), caches need body-aware cache keys, and most current cache implementations aren't built for that yet.
